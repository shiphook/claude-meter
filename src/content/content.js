...
// Prefer explicit used/limit; then utilization/util; then percent.
// If API exposes remaining*, convert to used fraction.
let util=null;
...
if (util==null){
const rem=num(b.remaining,b.remaining_fraction,b.remainingFraction,b.percent_remaining,b.percentRemaining);
if (rem!=null){
const remFrac=rem>1?rem/100:rem;
util=1-Math.min(1,Math.max(0,remFrac));
}
}
...
// Prefer explicit percent if it is on 0-100 used scale; if percent looks like remaining name, invert.
let pct=null;
const rawPct=num(b.percent,b.used_percent,b.usedPercent);
if (rawPct!=null){
if (String(present(b.percent_type,b.percentType,'')||'').toLowerCase().includes('remain')) pct=100-Math.min(100,Math.max(0,rawPct>1?rawPct:rawPct*100));
else pct=rawPct>1?Math.min(100,rawPct):rawPct*100;
}
